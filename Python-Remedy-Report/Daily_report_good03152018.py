...
#patternDel = '^[A]'
#filter = df1['Short Problem Description'].str.contains(patternDel)
#df1 = df1[~filter]
...
writer = pd.ExcelWriter('C:/Users/ab21600/Documents/Python-Remedy-Report/output.xlsx', engine='xlsxwriter')
Activation.to_excel(writer, sheet_name='Activation')
Maintenance.to_excel(writer, sheet_name='Maintenance')
Internal.to_excel(writer, sheet_name='Internal')
Outages.to_excel(writer, sheet_name='Outages')

#Skipping rows mean that it's skipping the first 5 rows
# Need to fid the first blank line skip that line and then append content
asset = pd.read_excel('output.xlsx', sheet_name='Activation', skiprows=5, 
              usecol='0', index_col=0)
