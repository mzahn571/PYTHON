...
# The juniper_bgp.dll is basically a txt file.  It represent the output of another script that runs in the crontab. 
# It basically pulls the BGP ipaddresses from a back configuration file.
...
# The juniper_bgp.dll is basically a txt file.  It represent the output of another script that runs in the crontab. 
# It basically pulls the VPN ipaddresses from a back configuration file.
...
# Templates should reflect real time not yesterday
template = '''
...
#  The ssh portion might not be required as the script will be define on the same server as the dll files
...
# Will require a change possble to another log name
...
# Parsing what is needed and putting it into a ipsec.log
    ...
    for line in end_of_pipe:
        word = line.strip()
        words=word
        # Parses the ipsec log looking for Idle\|OpenConfirm then place that pasrse info into juniper_bgp.log
...
## Parsing what is needed and to query against ipaddresses found in the ipsec configuration
    ...
    for line in end_of_pipe:
        word = line.strip()
        words=word
        # Parses the ipsec log looking for KMD_VPN_DOWN_ALARM_USER\|KMD_VPN_UP_ALARM_USER
        # then place that pasrse info into juniper_vpn.log
...
def CISCO_VPN_BGP_PARSING():
    ...
    # This capture might require a change to capture both up's and down's 
    p2 = subprocess.Popen(
    ...
    for line in end_of_pipe:
        word = line.strip()
        words=word
        # Parses the ipsec log looking for CRYPTO-5-IKEV2_SESSION_STATUS
        # then place that pasrse info into cisco_VPN_BGP.log
